...
//	FastNormalizeVector(aim.x, aim.y, aim.z, &aim);
	...
	flat.z = aim.z;
	flat.y = 0;
	OGLVector3D_Normalize(&flat, &flat);
...
OGLVector2D	aimVec,targetVec;
	...
	if (crossOut)
		*crossOut = cross;											// pass back cross product
...
OGLVector2D	aimVec,targetVec;
	...
	aimVec.y = -cos(r);
 	...
 	targetVec.x = gPlayerInfo[playerNum].coord.x - fromX;						// get normalized vector to target object
 	targetVec.y = gPlayerInfo[playerNum].coord.z - fromZ;
	OGLVector2D_Normalize(&targetVec, &targetVec);					// do ACCURATE normalize to avoid jitter!!!
	if ((targetVec.x == 0.0f) && (targetVec.y == 0.0f))
	{
		*turnDirection = 0;
...
{
	if (fabs(v1->x - v2->x) < 0.0001f)		// WARNING: If change, must change in BioOreoPro also!!
		if (fabs(v1->y - v2->y) < 0.0001f)
			if (fabs(v1->z - v2->z) < 0.0001f)
				return(true);
...
OGLVector3D	v;
	...
	v.x = (from->x * oneMinusRatio) + (to->x * ratio);				// interpolate it
	v.y = (from->y * oneMinusRatio) + (to->y * ratio);
	v.z = (from->z * oneMinusRatio) + (to->z * ratio);
...
/******************* FAST SIN/COS/TAN APPROXIMATIONS *************************/
//
