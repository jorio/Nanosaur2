...
public func GetQuadMeshWithin(_ theNode: UnsafeMutablePointer<ObjNode>!) -> UnsafeMutablePointer<MOVertexArrayData>! {
    ...
    SwGameAssert(metaObjectHeader.pointee.type == UInt32(MO_TYPE_GEOMETRY))
...
public func MakeQuadMeshObject(_ newObjDef: UnsafeMutablePointer<NewObjectDefinitionType>!, _ quadCapacity: Int32, _ material: UnsafeMutablePointer<MOMaterialObject>!) -> UnsafeMutablePointer<ObjNode>! {
    ...
    if material == nil {
        ...
        material = MO_CreateNewObjectOfType(UInt32(MO_TYPE_MATERIAL), 0, &matData)!.assumingMemoryBound(to: MOMaterialObject.self)
        ownMaterial = true
    }
    ...
    let meshMO = MO_CreateNewObjectOfType(UInt32(MO_TYPE_GEOMETRY), MO_GEOMETRY_SUBTYPE_VERTEXARRAY, &mesh)
