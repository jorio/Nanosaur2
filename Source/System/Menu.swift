...
//
// gEngine.menu.nav/gEngine.menu.outcome/gEngine.menu.numMenusRegistered/gMenuRegistry/kDefaultMenuStyle are
...
/// Menu-navigation state. Owned by GameEngine as `gEngine.menu`.
final class MenuSystem {
    var nav: UnsafeMutablePointer<MenuNavigation>!
    var outcome: Int32 = 0
    var numMenusRegistered: Int32 = 0

    fileprivate let registryBuf: UnsafeMutablePointer<UnsafePointer<MenuItem>?> = {
...
func GetRegistryEntry(_ i: Int32) -> UnsafePointer<MenuItem>? { gEngine.menu.registryBuf[Int(i)] }
func SetRegistryEntry(_ i: Int32, _ v: UnsafePointer<MenuItem>?) { gEngine.menu.registryBuf[Int(i)] = v }
...
@inline(__always) private func mObj(_ i: Int) -> UnsafeMutablePointer<ObjNode>? { Nav_GetMenuObject(gEngine.menu.nav, Int32(i)) }
@inline(__always) private func setMObj(_ i: Int, _ v: UnsafeMutablePointer<ObjNode>?) { Nav_SetMenuObject(gEngine.menu.nav, Int32(i), v) }
@inline(__always) private func mRowY(_ i: Int) -> Float { Nav_GetMenuRowY(gEngine.menu.nav, Int32(i)) }
@inline(__always) private func setMRowY(_ i: Int, _ v: Float) { Nav_SetMenuRowY(gEngine.menu.nav, Int32(i), v) }
@inline(__always) private func histMID(_ i: Int32) -> Int32 { Int32(Nav_GetHistoryMenuID(gEngine.menu.nav, i)) }
@inline(__always) private func setHistMID(_ i: Int32, _ v: Int32) { Nav_SetHistoryMenuID(gEngine.menu.nav, i, v) }
@inline(__always) private func histRow(_ i: Int32) -> Int32 { Nav_GetHistoryRow(gEngine.menu.nav, i) }
@inline(__always) private func setHistRow(_ i: Int32, _ v: Int32) { Nav_SetHistoryRow(gEngine.menu.nav, i, v) }
...
private func initMenuNavigation() {
    SwGameAssert(gEngine.menu.nav == nil)
    gEngine.menu.nav = AllocPtrClear(MemoryLayout<MenuNavigation>.size)!.assumingMemoryBound(to: MenuNavigation.self)
    let nav = gEngine.menu.nav!
...
private func disposeMenuNavigation() {
    ...
    gEngine.menu.nav = nil
}

// MARK: - Public API

func GetCurrentMenu() -> Int32 { gEngine.menu.nav != nil ? gEngine.menu.nav!.pointee.menuID : 0 }

func GetMenuIdleTime() -> Float { gEngine.menu.nav != nil ? gEngine.menu.nav!.pointee.idleTime : 0.0 }

func IsMenuMouseControlled() -> Bool { gEngine.menu.nav != nil ? gEngine.menu.nav!.pointee.mouseState != .off : false }

func GetCurrentMenuItemID() -> Int32 {
    guard let nav = gEngine.menu.nav, nav.pointee.focusRow >= 0 else { return -1 }
    return nav.pointee.menu![Int(nav.pointee.focusRow)].id
}

func GetCurrentMenuItemObject() -> UnsafeMutablePointer<ObjNode>? {
    guard let nav = gEngine.menu.nav, nav.pointee.focusRow >= 0 else { return nil }
...
func DisableEmptyFileSlots(_ menuItem: UnsafePointer<MenuItem>) -> Int32 {
    let mask = gEngine.menu.nav?.pointee.validSaveSlotMask ?? 0
...
func KillMenu(_ returnCode: Int32) {
    if gEngine.menu.nav!.pointee.menuState == .ready {
        gEngine.menu.nav!.pointee.menuPick = returnCode; gEngine.menu.nav!.pointee.menuState = .fadeOut
    }
}

func LayoutCurrentMenuAgain(_ animate: Bool) {
    SwGameAssert(gEngine.menu.nav != nil); SwGameAssert(gEngine.menu.nav!.pointee.menu != nil)
    gEngine.menu.nav!.pointee.sweepDelay = animate ? 0 : -100000
    saveSelectedRowInHistory()
    layOutMenu(gEngine.menu.nav!.pointee.menuID)
}
...
func RegisterMenu(_ menuTree: UnsafePointer<MenuItem>) {
    ...
    while mi.pointee.type != MenuItemType.sentinel || mi.pointee.id != 0 {
        if mi.pointee.type == MenuItemType.sentinel {
            ...
            SwGameAssert(gEngine.menu.numMenusRegistered < MAX_REGISTERED_MENUS)
            SetRegistryEntry(gEngine.menu.numMenusRegistered, mi)
            gEngine.menu.numMenusRegistered += 1
        }
        mi = mi.successor()
    }
}
...
func MakeMenu(_ menu: UnsafePointer<MenuItem>, _ style: UnsafePointer<MenuStyle>?) -> UnsafeMutablePointer<ObjNode> {
    ...
    gEngine.menu.numMenusRegistered = 0
    RegisterMenu(menu)
    initMenuNavigation()
    gEngine.menu.outcome = 0
    gEngine.menu.nav!.pointee.menuState = .fadeIn; gEngine.menu.nav!.pointee.menuFadeAlpha = 0; gEngine.menu.nav!.pointee.focusRow = -1
    if let style { SDL_memcpy(&gEngine.menu.nav!.pointee.style, style, MemoryLayout<MenuStyle>.size) }
    gEngine.menu.nav!.pointee.sweepDelay = 0; gEngine.menu.nav!.pointee.sweepRTL = false
    gEngine.menu.nav!.pointee.darkenPane = nil
    if gEngine.menu.nav!.pointee.style.darkenPaneOpacity > 0.0 { gEngine.menu.nav!.pointee.darkenPane = makeDarkenPane() }
...
private func getMenuItemHeight(_ row: Int32) -> Float {
    let mi = gEngine.menu.nav!.pointee.menu!.advanced(by: Int(row))
...
private func pulsateColor(_ time: UnsafeMutablePointer<Float>!) -> OGLColorRGBA {
    time.pointee += gEngine.framesPerSecondFrac
...
private func getBindingAtRow(_ row: Int32) -> UnsafeMutablePointer<InputBinding> {
    let need = Int(MenuItem_GetInputNeed(gEngine.menu.nav!.pointee.menu!.advanced(by: Int(row))))
...
private func getCurrentInteractableMenuItemObject() -> UnsafeMutablePointer<ObjNode>? {
    ...
    let nav = gEngine.menu.nav!; let t = nav.pointee.menu![Int(nav.pointee.focusRow)].type
    switch t {
...
private func twitchSelectionInOrOut(_ scaleIn: Bool) {
    ...
    let s = getMenuItemHeight(gEngine.menu.nav!.pointee.focusRow) * gEngine.menu.nav!.pointee.style.standardScale
...
private func twitchSelectionNopeWiggle() {
    ...
    _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 0), Int32(kTwitchPreset_PadlockWiggle))
    _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 1), Int32(kTwitchPreset_PadlockWiggle))
    playErrorEffect()
}
...
private let cMoveDarkenPane: @convention(c) (UnsafeMutablePointer<ObjNode>?) -> Void = { node in
    ...
    var t = node.pointee.Timer + gEngine.framesPerSecondFrac * Float(5.0); if t > 1 { t = 1 }
...
private func makeDarkenPane() -> UnsafeMutablePointer<ObjNode> {
    ...
    def.slot = Int16(gEngine.menu.nav!.pointee.style.textSlot - 1); def.scale = 1
    def.coord = (OGLPoint3D(x: kDefaultX, y: gEngine.menu.nav!.pointee.style.yOffset, z: 0))
    def.moveCall = cMoveDarkenPane; def.drawCall = cDrawDarkenPane
    let pane = MakeNewObject(&def)!
    pane.pointee.Scale.y = Float(EPS)
    pane.pointee.ColorFilter = OGLColorRGBA(r: 0, g: 0, b: 0, a: gEngine.menu.nav!.pointee.style.darkenPaneOpacity)
...
private let cMoveAction: @convention(c) (UnsafeMutablePointer<ObjNode>?) -> Void = { node in
    guard let node = node else { return }
    if getMenuNodeData(node).pointee.row == gEngine.menu.nav!.pointee.focusRow { node.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.highlightColor }
    else { node.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.idleColor }
...
private let cMoveControlBinding: @convention(c) (UnsafeMutablePointer<ObjNode>?) -> Void = { node in
    ...
    let miType = gEngine.menu.nav!.pointee.menu![Int(data.pointee.row)].type
    ...
    if data.pointee.row == gEngine.menu.nav!.pointee.focusRow && data.pointee.component == gEngine.menu.nav!.pointee.focusComponent {
        if gEngine.menu.nav!.pointee.menuState == ps { node.pointee.ColorFilter = pulsateColor(&data.pointee.pulsateTimer) }
        else { node.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.highlightColor }
    } else { node.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.idleColor }
...
private func saveSelectedRowInHistory() { setHistRow(gEngine.menu.nav!.pointee.historyPos, gEngine.menu.nav!.pointee.focusRow) }
private func goBackInHistory() {
    MyFlushEvents()
    if gEngine.menu.nav!.pointee.historyPos != 0 {
        playBackEffect(); gEngine.menu.nav!.pointee.historyPos -= 1
        gEngine.menu.nav!.pointee.sweepRTL = true; layOutMenu(histMID(gEngine.menu.nav!.pointee.historyPos))
        gEngine.menu.nav!.pointee.sweepRTL = false
    } else if MenuStyle_GetCanBackOutOfRootMenu(&gEngine.menu.nav!.pointee.style) { playBackEffect(); gEngine.menu.nav!.pointee.menuState = .fadeOut }
...
private func navigateMenu() {
    SwGameAssert(MenuStyle_GetIsInteractive(&gEngine.menu.nav!.pointee.style))
    if SwIsNeedDown(kNeed_UIBack, ANY_PLAYER) || SwIsClickDown(Int(SDL_BUTTON_X1)) { goBackInHistory(); return }
    if SwIsNeedDown(kNeed_UIUp, ANY_PLAYER) { navigateMenuVertically(-1); saveSelectedRowInHistory(); gEngine.menu.nav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UIDown, ANY_PLAYER) { navigateMenuVertically(1); saveSelectedRowInHistory(); gEngine.menu.nav!.pointee.mouseState = .off }
    else { navigateMenuMouseHover() }
    let entry = gEngine.menu.nav!.pointee.menu!.advanced(by: Int(gEngine.menu.nav!.pointee.focusRow))
    navigateMenuItem(entry.pointee.type, entry)
}
private func navigateMenuVertically(_ delta: Int32) {
    let makeSound = gEngine.menu.nav!.pointee.focusRow >= 0; var browsed = 0
    twitchOutSelection()
    var skip = true
    while skip {
        gEngine.menu.nav!.pointee.focusRow += delta
        gEngine.menu.nav!.pointee.focusRow = Int32(PositiveModulo(gEngine.menu.nav!.pointee.focusRow, UInt32(gEngine.menu.nav!.pointee.numRows)))
        skip = !isMenuItemSelectable(gEngine.menu.nav!.pointee.menu!.advanced(by: Int(gEngine.menu.nav!.pointee.focusRow)))
        browsed += 1; if browsed > gEngine.menu.nav!.pointee.numRows { return }
    }
    gEngine.menu.nav!.pointee.idleTime = 0
    if makeSound { playNavigateEffect(); twitchSelection() }
}
private func navigateMenuMouseHover() {
    if gEngine.menu.nav!.pointee.mouseState == .grabbing { return }
    ...
    gEngine.menu.nav!.pointee.mouseState = .wandering; gEngine.menu.nav!.pointee.mouseFocusComponent = -1
    for row in 0..<gEngine.menu.nav!.pointee.numRows {
        let mi = gEngine.menu.nav!.pointee.menu!.advanced(by: Int(row))
        ...
        if cursor.y >= fe.top && cursor.y <= fe.bottom && cursor.x >= fe.left - kMouseHoverTolerance && cursor.x <= fe.right + kMouseHoverTolerance {
            gEngine.menu.nav!.pointee.mouseState = .hovering; gEngine.menu.nav!.pointee.mouseFocusComponent = fc
            if gEngine.menu.nav!.pointee.focusRow != Int32(row) {
                gEngine.menu.nav!.pointee.idleTime = 0; twitchSelectionInOrOut(false)
                gEngine.menu.nav!.pointee.focusRow = Int32(row); playNavigateEffect(); twitchSelectionInOrOut(true)
            }
            return
        }
    }
...
private func updateArrows() {
    ...
    case MenuItemType.keyBinding, MenuItemType.padBinding:
        visible = [true, true]
        if gEngine.menu.nav!.pointee.menuState == .awaitingKeyPress || gEngine.menu.nav!.pointee.menuState == .awaitingPadPress || gEngine.menu.nav!.pointee.menuState == .awaitingMouseClick { visible = [false, false] }
    ...
    if gEngine.menu.nav!.pointee.mouseState != .off { snapTo = nil }
    guard let snapTo = snapTo else { for i in 0..<2 { SetObjectVisible(Nav_GetArrow(gEngine.menu.nav!, Int32(i))!, 0) }; return }
    let ext = TextMesh_GetExtents(snapTo); let spacing: Float = 45 * snapTo.pointee.Scale.x
    for i in 0..<2 {
        let a = Nav_GetArrow(gEngine.menu.nav!, Int32(i))!; SetObjectVisible(a, 1)
    ...
    for i in 0..<2 { UpdateObjectTransforms(Nav_GetArrow(gEngine.menu.nav!, Int32(i))!) }
}

// MARK: - Widget layouts

private func layOutLabel(_ row: Int32) -> UnsafeMutablePointer<ObjNode> {
    let e = gEngine.menu.nav!.pointee.menu!.advanced(by: Int(row))
    let l = makeText(getMenuItemText(e), row, 0, kTextMeshSmallCapsFlag | kTextMeshUserFlag_AltFont)
    l.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.labelColor; l.pointee.MoveCall = cMoveLabel
...
private func navigatePick(_ entry: UnsafePointer<MenuItem>!) {
    let validClick = gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || validClick {
        if !validClick { gEngine.menu.nav!.pointee.mouseState = .off }
        if entry.pointee.next != BACK_FOURCC { playConfirmEffect() }
        gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.menuPick = entry.pointee.id
        if let cb = entry.pointee.callback { cb() }
        switch entry.pointee.next {
        case 0, NOOP_FOURCC: twitchSelection()
        case EXIT_FOURCC: gEngine.menu.nav!.pointee.menuState = .fadeOut
        case BACK_FOURCC: goBackInHistory()
        default:
            saveSelectedRowInHistory(); gEngine.menu.nav!.pointee.historyPos += 1
            setHistMID(gEngine.menu.nav!.pointee.historyPos, entry.pointee.next); setHistRow(gEngine.menu.nav!.pointee.historyPos, 0)
...
private func navigateCycler(_ e: UnsafePointer<MenuItem>!) {
    ...
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { delta = -1; gEngine.menu.nav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { delta = 1; gEngine.menu.nav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) { gEngine.menu.nav!.pointee.mouseState = .off; twitchSelectionNopeWiggle(); return }
    else if gEngine.menu.nav!.pointee.mouseState == .hovering {
    ...
    if delta != 0 {
        gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.sweepRTL = delta == -1
        ...
        if e.pointee.type == MenuItemType.cycler1 { _ = layOutCycler1Column(gEngine.menu.nav!.pointee.focusRow) }
        else { _ = layOutCycler2ColumnsValueText(gEngine.menu.nav!.pointee.focusRow) }
        _ = MakeTwitch(delta < 0 ? Nav_GetArrow(gEngine.menu.nav!, 0) : Nav_GetArrow(gEngine.menu.nav!, 1), Int32(delta < 0 ? kTwitchPreset_DisplaceLTR : kTwitchPreset_DisplaceRTL))
    }
}
...
private func navigateSlider(_ e: UnsafePointer<MenuItem>!) {
    ...
    if gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT)) {
        switch gEngine.menu.nav!.pointee.mouseFocusComponent {
        ...
        if beingDragged {
            ...
            gEngine.menu.nav!.pointee.mouseState = .grabbing; playStartBindingEffect(); twitchSelection()
        }
        ...
        beingDragged = false
        if gEngine.menu.nav!.pointee.mouseState == .grabbing { gEngine.menu.nav!.pointee.mouseState = .wandering }
        playConfirmEffect(); if let cb = e.pointee.callback { cb() }
    } else if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) {
        _ = MakeTwitch(getCurrentInteractableMenuItemObject(), Int32(kTwitchPreset_MenuSelect))
        _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 0), Int32(kTwitchPreset_PadlockWiggle))
        _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 1), Int32(kTwitchPreset_PadlockWiggle)); playErrorEffect()
        ...
        if didJustPress { didRamEdge = false }
            ...
            heldCooldown -= gEngine.framesPerSecondFrac * Float(12) * (GetNeedAnalogValue(Int32(kNeed_UIPrev), Int32(ANY_PLAYER)) + GetNeedAnalogValue(Int32(kNeed_UINext), Int32(ANY_PLAYER)))
        ...
        if (dir < 0 && vp.pointee == MenuItem_GetSliderMin(e)) || (dir > 0 && vp.pointee == MenuItem_GetSliderMax(e)) {
            if !didRamEdge {
                _ = MakeTwitch(getCurrentInteractableMenuItemObject(), Int32(kTwitchPreset_MenuSelect))
                _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 0), Int32(kTwitchPreset_PadlockWiggle))
                _ = MakeTwitch(Nav_GetArrow(gEngine.menu.nav!, 1), Int32(kTwitchPreset_PadlockWiggle)); playErrorEffect(); didRamEdge = true
            }
        } else {
            gEngine.menu.nav!.pointee.mouseState = .off; let inc = Float(MenuItem_GetSliderIncrement(e))
            ...
            _ = MakeTwitch(dir < 0 ? Nav_GetArrow(gEngine.menu.nav!, 0) : Nav_GetArrow(gEngine.menu.nav!, 1), Int32(dir < 0 ? kTwitchPreset_DisplaceLTR : kTwitchPreset_DisplaceRTL))
        }
    }
}
...
private func layOutFileSlot(_ row: Int32) -> UnsafeMutablePointer<ObjNode> {
    ...
    if valid != 0 { gEngine.menu.nav!.pointee.validSaveSlotMask |= (1 << UInt64(MenuItem_GetFileSlot(e))) }
    let buf2: String
...
private func layOutKeyBinding(_ row: Int32) -> UnsafeMutablePointer<ObjNode> {
    ...
    l.pointee.Coord.x = 100; l.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.labelColor; l.pointee.MoveCall = cMoveAction
...
private func layOutMouseBinding(_ row: Int32) -> UnsafeMutablePointer<ObjNode> {
    let e = gEngine.menu.nav!.pointee.menu!.advanced(by: Int(row))
    let l = makeText("\(localized(LocStrID(rawValue: STR_KEYBINDING_DESCRIPTION_0.rawValue + UInt32(MenuItem_GetInputNeed(e))))):", row, 0, kTextMeshAlignLeftFlag | kTextMeshSmallCapsFlag | kTextMeshUserFlag_AltFont)
    l.pointee.Coord.x = k2ColumnLeftX; l.pointee.ColorFilter = gEngine.menu.nav!.pointee.style.labelColor; l.pointee.MoveCall = cMoveAction
...
private func makeKbText(_ row: Int32, _ keyNo: Int32) -> UnsafeMutablePointer<ObjNode> {
    let name: String = gEngine.menu.nav!.pointee.menuState == .awaitingKeyPress ? localized(STR_PRESS) : getKeyBindingName(row, keyNo)
...
private func makePbText(_ row: Int32, _ btnNo: Int32) -> UnsafeMutablePointer<ObjNode> {
    let name: String = gEngine.menu.nav!.pointee.menuState == .awaitingPadPress ? localized(STR_PRESS) : getPadBindingName(row, btnNo)
...
private func makeMbText(_ row: Int32) -> UnsafeMutablePointer<ObjNode> {
    let name: String = gEngine.menu.nav!.pointee.menuState == .awaitingMouseClick ? localized(STR_CLICK) : getMouseBindingName(row)
...
private func navigateKeyBinding(_ e: UnsafePointer<MenuItem>!) {
    var keyNo = Int32(PositiveModulo(gEngine.menu.nav!.pointee.focusComponent - 1, UInt32(MAX_USER_BINDINGS_PER_NEED)))
    gEngine.menu.nav!.pointee.focusComponent = keyNo + 1; let row = gEngine.menu.nav!.pointee.focusRow
    if gEngine.menu.nav!.pointee.mouseState == .hovering && (gEngine.menu.nav!.pointee.mouseFocusComponent == 1 || gEngine.menu.nav!.pointee.mouseFocusComponent == 2) && keyNo != gEngine.menu.nav!.pointee.mouseFocusComponent - 1 {
        keyNo = gEngine.menu.nav!.pointee.mouseFocusComponent - 1; twitchOutSelection(); gEngine.menu.nav!.pointee.idleTime = 0
        gEngine.menu.nav!.pointee.focusComponent = keyNo + 1; playNavigateEffect(); twitchSelection(); return
    }
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { twitchOutSelection(); keyNo = Int32(PositiveModulo(keyNo - 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.focusComponent = keyNo + 1; playNavigateEffect(); gEngine.menu.nav!.pointee.mouseState = .off; twitchSelection(); return }
    if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { twitchOutSelection(); keyNo = Int32(PositiveModulo(keyNo + 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.focusComponent = keyNo + 1; playNavigateEffect(); gEngine.menu.nav!.pointee.mouseState = .off; twitchSelection(); return }
    if SwIsNeedDown(kNeed_UIDelete, ANY_PLAYER) || SwIsKeyDown(Int(CLEAR_BINDING_SCANCODE)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_MIDDLE))) {
        twitchOutSelection(); gEngine.menu.nav!.pointee.idleTime = 0; InputBinding_SetKey(getBindingAtRow(row), keyNo, 0); playDeleteEffect(); _ = makeKbText(row, keyNo); twitchSelection(); return
    }
    if SwIsKeyDown(Int(SDL_SCANCODE_RETURN.rawValue)) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); InvalidateAllInputs(); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.menuState = .awaitingKeyPress; _ = makeKbText(row, keyNo); twitchSelection(); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP_CANCEL); return
    }
}
private func navigatePadBinding(_ e: UnsafePointer<MenuItem>!) {
    var btnNo = Int32(PositiveModulo(gEngine.menu.nav!.pointee.focusComponent - 1, UInt32(MAX_USER_BINDINGS_PER_NEED)))
    gEngine.menu.nav!.pointee.focusComponent = btnNo + 1; let row = gEngine.menu.nav!.pointee.focusRow
    if gEngine.menu.nav!.pointee.mouseState == .hovering && (gEngine.menu.nav!.pointee.mouseFocusComponent == 1 || gEngine.menu.nav!.pointee.mouseFocusComponent == 2) && btnNo != gEngine.menu.nav!.pointee.mouseFocusComponent - 1 {
        btnNo = gEngine.menu.nav!.pointee.mouseFocusComponent - 1; twitchOutSelection(); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.focusComponent = btnNo + 1; playNavigateEffect(); twitchSelection(); return
    }
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { twitchOutSelection(); btnNo = Int32(PositiveModulo(btnNo - 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gEngine.menu.nav!.pointee.focusComponent = btnNo + 1; gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.mouseState = .off; playNavigateEffect(); twitchSelection(); return }
    if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { twitchOutSelection(); btnNo = Int32(PositiveModulo(btnNo + 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gEngine.menu.nav!.pointee.focusComponent = btnNo + 1; gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.mouseState = .off; playNavigateEffect(); twitchSelection(); return }
    if SwIsNeedDown(kNeed_UIDelete, ANY_PLAYER) || SwIsKeyDown(Int(CLEAR_BINDING_SCANCODE)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_MIDDLE))) {
        twitchOutSelection(); gEngine.menu.nav!.pointee.idleTime = 0; InputBinding_SetPad(getBindingAtRow(row), btnNo, Int8(kInputTypeUnbound), 0); playDeleteEffect(); _ = makePbText(row, btnNo); twitchSelection(); return
    }
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.menuState = .awaitingPadPress; _ = makePbText(row, btnNo); twitchSelection(); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP_CANCEL); return
    }
}
private func navigateMouseBinding(_ e: UnsafePointer<MenuItem>!) {
    let row = gEngine.menu.nav!.pointee.focusRow; gEngine.menu.nav!.pointee.focusComponent = 1
    if SwIsNeedDown(kNeed_UIDelete, ANY_PLAYER) || SwIsKeyDown(Int(CLEAR_BINDING_SCANCODE)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_MIDDLE))) {
        gEngine.menu.nav!.pointee.idleTime = 0; getBindingAtRow(row).pointee.mouseButton = 0; playDeleteEffect(); _ = makeMbText(row); twitchSelection(); return
    }
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gEngine.menu.nav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.menuState = .awaitingMouseClick; _ = makeMbText(row); twitchSelection(); InvalidateAllInputs(); return
    }
}
...
private func unbindScancodeFromAllRemappableInputNeeds(_ sc: Int16) {
    for row in 0..<gEngine.menu.nav!.pointee.numRows {
        if gEngine.menu.nav!.pointee.menu![Int(row)].type != MenuItemType.keyBinding { continue }
        let b = getBindingAtRow(row)
...
private func unbindPadButtonFromAllRemappableInputNeeds(_ type: Int8, _ id: Int8) {
    for row in 0..<gEngine.menu.nav!.pointee.numRows {
        if gEngine.menu.nav!.pointee.menu![Int(row)].type != MenuItemType.padBinding { continue }
        let b = getBindingAtRow(row)
...
private func unbindMouseButtonFromAllRemappableInputNeeds(_ id: Int8) {
    for row in 0..<gEngine.menu.nav!.pointee.numRows {
        if gEngine.menu.nav!.pointee.menu![Int(row)].type != MenuItemType.mouseBinding { continue }
        let b = getBindingAtRow(row)
...
private func awaitKeyPress() {
    let row = gEngine.menu.nav!.pointee.focusRow; let keyNo = gEngine.menu.nav!.pointee.focusComponent - 1
    var doWiggle = false
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SwIsClickDown(Int(SDL_BUTTON_LEFT)) || SwIsClickDown(Int(SDL_BUTTON_RIGHT)) { doWiggle = true }
        ...
        for sc: Int16 in 0..<Int16(SDL_SCANCODE_COUNT.rawValue) {
            if SwIsKeyDown(Int(sc)) {
                ...
                gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makeKbText(row, keyNo); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP)
                return
            }
        }
        return
    }
    gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makeKbText(row, keyNo); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP); if doWiggle { twitchSelectionNopeWiggle() }
}
private func awaitGamepadPress(_ controller: OpaquePointer) -> Bool {
    let row = gEngine.menu.nav!.pointee.focusRow; let btnNo = gEngine.menu.nav!.pointee.focusComponent - 1
    var doWiggle = false
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SDL_GetGamepadButton(controller, SDL_GAMEPAD_BUTTON_START) || SwIsClickDown(Int(SDL_BUTTON_LEFT)) || SwIsClickDown(Int(SDL_BUTTON_RIGHT)) { doWiggle = true }
        ...
        let b = getBindingAtRow(gEngine.menu.nav!.pointee.focusRow)
        for button: Int8 in 0..<Int8(SDL_GAMEPAD_BUTTON_COUNT.rawValue) {
            if SDL_GetGamepadButton(controller, SDL_GamepadButton(rawValue: Int32(button))) {
                ...
                gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP); return true
            }
        }
    ...
    gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP); if doWiggle { twitchSelectionNopeWiggle() }; return true
}
private func awaitMetaGamepadPress() {
    ...
    if !anyFound { gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_NO_GAMEPAD_DETECTED); twitchSelectionNopeWiggle() }
}
private func awaitMouseClick() {
    ...
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsNeedDown(kNeed_UIBack, ANY_PLAYER) { doWiggle = true }
        ...
        for mb: Int8 in 0..<Int8(NUM_SUPPORTED_MOUSE_BUTTONS) {
            if SwIsClickDown(Int(mb)) { unbindMouseButtonFromAllRemappableInputNeeds(mb); b.pointee.mouseButton = mb; playEndBindingEffect(); gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makeMbText(gEngine.menu.nav!.pointee.focusRow); return }
        }
        return
    }
    gEngine.menu.nav!.pointee.menuState = .ready; gEngine.menu.nav!.pointee.idleTime = 0; _ = makeMbText(gEngine.menu.nav!.pointee.focusRow); if doWiggle { twitchSelectionNopeWiggle() }
}
...
private func makeText(_ text: String, _ row: Int32, _ chainItem: Int32, _ textMeshFlags: Int32) -> UnsafeMutablePointer<ObjNode> {
    ...
    var fontAtlas = gEngine.menu.nav!.pointee.style.fontAtlas
    if (textMeshFlags & kTextMeshUserFlag_AltFont) != 0 { fontAtlas = gEngine.menu.nav!.pointee.style.fontAtlas2 }
    if node != nil {
        ...
        def.scale = getMenuItemHeight(row) * gEngine.menu.nav!.pointee.style.standardScale
        def.group = UInt8(fontAtlas); def.slot = gEngine.menu.nav!.pointee.style.textSlot + Int16(chainItem); def.flags = UInt32(STATUS_BIT_MOVEINPAUSE)
    ...
    if gEngine.menu.nav!.pointee.sweepDelay >= 0 {
        if let fe = MakeTwitch(node, Int32(kTwitchPreset_MenuFadeIn)) {
            fe.pointee.delay = gEngine.menu.nav!.pointee.sweepDelay * fe.pointee.duration
            if let de = MakeTwitch(node, Int32(kTwitchPreset_MenuSweep | kTwitchFlags_Chain)) {
                de.pointee.delay = fe.pointee.delay; de.pointee.amplitude *= (gEngine.menu.nav!.pointee.sweepRTL ? 1 : -1); de.pointee.amplitude *= Float(1.0 + Double(de.pointee.delay))
            }
        }
    }
    return node!
...
private func replaceMenuText(_ origID: LocStrID, _ newText: LocStrID) {
    for i in 0..<Int(MAX_MENU_ROWS) {
        let mi = gEngine.menu.nav!.pointee.menu!.advanced(by: i)
...
private func layOutMenu(_ menuID: Int32) {
    ...
    setHistMID(gEngine.menu.nav!.pointee.historyPos, menuID)
    gEngine.menu.nav!.pointee.menu = menu; gEngine.menu.nav!.pointee.menuID = menuID; gEngine.menu.nav!.pointee.menuPick = -1
    gEngine.menu.nav!.pointee.numRows = 0; gEngine.menu.nav!.pointee.idleTime = 0; gEngine.menu.nav!.pointee.validSaveSlotMask = 0
    deleteAllText()
    var row = 0; while menu[row].type != MenuItemType.sentinel { gEngine.menu.nav!.pointee.numRows += 1; SwGameAssert(gEngine.menu.nav!.pointee.numRows <= MAX_MENU_ROWS); row += 1 }
    var totalHeight: Float = 0; var firstHeight: Float = 0; row = 0
    while menu[row].type != MenuItemType.sentinel { let h = getMenuItemHeight(Int32(row)); if firstHeight == 0 { firstHeight = h }; totalHeight += h * gEngine.menu.nav!.pointee.style.rowHeight; row += 1 }
    var y = -totalHeight * Float(0.5) + gEngine.menu.nav!.pointee.style.yOffset; y += firstHeight * gEngine.menu.nav!.pointee.style.rowHeight / Float(2.0)
    if let dp = gEngine.menu.nav!.pointee.darkenPane { rescaleDarkenPane(dp, totalHeight) }
    row = 0
    while menu[row].type != MenuItemType.sentinel {
        ...
        y += getMenuItemHeight(Int32(row)) * gEngine.menu.nav!.pointee.style.rowHeight
        if et != MenuItemType.spacer { gEngine.menu.nav!.pointee.sweepDelay += Float(0.2) }
        row += 1
    }
    gEngine.menu.nav!.pointee.sweepDelay = 0.0
    gEngine.menu.nav!.pointee.focusRow = histRow(gEngine.menu.nav!.pointee.historyPos)
    if gEngine.menu.nav!.pointee.focusRow <= 0 || !isMenuItemSelectable(gEngine.menu.nav!.pointee.menu!.advanced(by: Int(gEngine.menu.nav!.pointee.focusRow))) { gEngine.menu.nav!.pointee.focusRow = -1; navigateMenuVertically(1) }
    twitchSelection()
...
private func lookUpMenu(_ menuID: Int32) -> UnsafePointer<MenuItem>? {
    for i in 0..<gEngine.menu.numMenusRegistered { let e = GetRegistryEntry(i); if e!.pointee.id == menuID { return e } }
    return nil
}
...
private let cMoveMenuDriver: @convention(c) (UnsafeMutablePointer<ObjNode>?) -> Void = { theNode in
    ...
    if gEngine.menu.nav!.pointee.menuState == .off { return }
    gEngine.menu.nav!.pointee.idleTime += gEngine.framesPerSecondFrac
    if SwIsNeedDown(kNeed_UIStart, ANY_PLAYER) && MenuStyle_GetStartButtonExits(&gEngine.menu.nav!.pointee.style) && MenuStyle_GetCanBackOutOfRootMenu(&gEngine.menu.nav!.pointee.style)
        && gEngine.menu.nav!.pointee.menuState != .awaitingPadPress && gEngine.menu.nav!.pointee.menuState != .awaitingKeyPress && gEngine.menu.nav!.pointee.menuState != .awaitingMouseClick {
        gEngine.menu.nav!.pointee.menuState = .fadeOut
    }
    switch gEngine.menu.nav!.pointee.menuState {
    case .fadeIn:
        gEngine.menu.nav!.pointee.menuFadeAlpha += gEngine.framesPerSecondFrac * gEngine.menu.nav!.pointee.style.fadeInSpeed
        if gEngine.menu.nav!.pointee.menuFadeAlpha >= 1.0 { gEngine.menu.nav!.pointee.menuFadeAlpha = 1.0; gEngine.menu.nav!.pointee.menuState = .ready }
    case .fadeOut:
        if MenuStyle_GetAsyncFadeOut(&gEngine.menu.nav!.pointee.style) { gEngine.menu.nav!.pointee.menuState = .off }
        else { gEngine.menu.nav!.pointee.menuFadeAlpha -= gEngine.framesPerSecondFrac * gEngine.menu.nav!.pointee.style.fadeOutSpeed; if gEngine.menu.nav!.pointee.menuFadeAlpha <= 0 { gEngine.menu.nav!.pointee.menuFadeAlpha = 0; gEngine.menu.nav!.pointee.menuState = .off } }
    case .ready:
        if MenuStyle_GetIsInteractive(&gEngine.menu.nav!.pointee.style) { navigateMenu() }
        else if SwIsNeedDown(kNeed_UIBack, ANY_PLAYER) || SwIsNeedDown(kNeed_UIPause, ANY_PLAYER) { goBackInHistory() }
    case .awaitingKeyPress: awaitKeyPress()
    case .awaitingPadPress: awaitMetaGamepadPress()
    case .awaitingMouseClick: awaitMouseClick()
    default: break
    }
    updateArrows()
    if gEngine.menu.nav!.pointee.menuState == .off { cleanUpMenuDriver(theNode) }
}

private func cleanUpMenuDriver(_ theNode: UnsafeMutablePointer<ObjNode>!) {
    if MenuStyle_GetFadeOutSceneOnExit(&gEngine.menu.nav!.pointee.style) { OGL_FadeOutScene(DrawObjects, nil) }
    if MenuStyle_GetAsyncFadeOut(&gEngine.menu.nav!.pointee.style) {
        if let dp = gEngine.menu.nav!.pointee.darkenPane { rescaleDarkenPane(dp, Float(EPS)); _ = MakeTwitch(dp, Int32(kTwitchPreset_MenuFadeOut | kTwitchFlags_KillPuppet)) }
        ...
        deleteAllText()
        if let dp = gEngine.menu.nav!.pointee.darkenPane { DeleteObject(dp); gEngine.menu.nav!.pointee.darkenPane = nil }
    }
    DoSDLMaintenance(); MyFlushEvents()
    let exitCall = gEngine.menu.nav!.pointee.style.exitCall
    gEngine.menu.outcome = gEngine.menu.nav!.pointee.menuPick
    disposeMenuNavigation(); DeleteObject(theNode)
    if let ec = exitCall { ec(gEngine.menu.outcome) }
}
