...
func MakeMenu(_ menu: UnsafePointer<MenuItem>, _ style: UnsafePointer<MenuStyle>?) -> UnsafeMutablePointer<ObjNode> {
    ...
    initMenuNavigation()
    gMenuOutcome = 0
    gNav!.pointee.menuState = .fadeIn; gNav!.pointee.menuFadeAlpha = 0; gNav!.pointee.focusRow = -1
...
private func goBackInHistory() {
    ...
    } else if MenuStyle_GetCanBackOutOfRootMenu(&gNav!.pointee.style) { playBackEffect(); gNav!.pointee.menuState = .fadeOut }
...
private func navigateMenu() {
    ...
    if SwIsNeedDown(kNeed_UIUp, ANY_PLAYER) { navigateMenuVertically(-1); saveSelectedRowInHistory(); gNav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UIDown, ANY_PLAYER) { navigateMenuVertically(1); saveSelectedRowInHistory(); gNav!.pointee.mouseState = .off }
...
private func navigateMenuMouseHover() {
    ...
    for row in 0..<gNav!.pointee.numRows {
        ...
        if cursor.y >= fe.top && cursor.y <= fe.bottom && cursor.x >= fe.left - kMouseHoverTolerance && cursor.x <= fe.right + kMouseHoverTolerance {
            gNav!.pointee.mouseState = .hovering; gNav!.pointee.mouseFocusComponent = fc
...
private func navigatePick(_ entry: UnsafePointer<MenuItem>!) {
    let validClick = gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || validClick {
        if !validClick { gNav!.pointee.mouseState = .off }
        ...
        case EXIT_FOURCC: gNav!.pointee.menuState = .fadeOut
...
private func navigateCycler(_ e: UnsafePointer<MenuItem>!) {
    ...
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { delta = -1; gNav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { delta = 1; gNav!.pointee.mouseState = .off }
    else if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) { gNav!.pointee.mouseState = .off; twitchSelectionNopeWiggle(); return }
    else if gNav!.pointee.mouseState == .hovering {
...
private func navigateSlider(_ e: UnsafePointer<MenuItem>!) {
    ...
    if gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT)) {
        ...
        beingDragged = false
        if gNav!.pointee.mouseState == .grabbing { gNav!.pointee.mouseState = .wandering }
        ...
        if (dir < 0 && vp.pointee == MenuItem_GetSliderMin(e)) || (dir > 0 && vp.pointee == MenuItem_GetSliderMax(e)) {
            ...
            gNav!.pointee.mouseState = .off; let inc = Float(MenuItem_GetSliderIncrement(e))
...
private func navigateKeyBinding(_ e: UnsafePointer<MenuItem>!) {
    ...
    if gNav!.pointee.mouseState == .hovering && (gNav!.pointee.mouseFocusComponent == 1 || gNav!.pointee.mouseFocusComponent == 2) && keyNo != gNav!.pointee.mouseFocusComponent - 1 {
    ...
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { twitchOutSelection(); keyNo = Int32(PositiveModulo(keyNo - 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gNav!.pointee.idleTime = 0; gNav!.pointee.focusComponent = keyNo + 1; playNavigateEffect(); gNav!.pointee.mouseState = .off; twitchSelection(); return }
    if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { twitchOutSelection(); keyNo = Int32(PositiveModulo(keyNo + 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gNav!.pointee.idleTime = 0; gNav!.pointee.focusComponent = keyNo + 1; playNavigateEffect(); gNav!.pointee.mouseState = .off; twitchSelection(); return }
    if SwIsNeedDown(kNeed_UIDelete, ANY_PLAYER) || SwIsKeyDown(Int(CLEAR_BINDING_SCANCODE)) || (gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_MIDDLE))) {
    ...
    if SwIsKeyDown(Int(SDL_SCANCODE_RETURN.rawValue)) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); InvalidateAllInputs(); gNav!.pointee.idleTime = 0; gNav!.pointee.menuState = .awaitingKeyPress; _ = makeKbText(row, keyNo); twitchSelection(); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP_CANCEL); return
    }
}
private func navigatePadBinding(_ e: UnsafePointer<MenuItem>!) {
    ...
    if gNav!.pointee.mouseState == .hovering && (gNav!.pointee.mouseFocusComponent == 1 || gNav!.pointee.mouseFocusComponent == 2) && btnNo != gNav!.pointee.mouseFocusComponent - 1 {
    ...
    if SwIsNeedDown(kNeed_UIPrev, ANY_PLAYER) { twitchOutSelection(); btnNo = Int32(PositiveModulo(btnNo - 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gNav!.pointee.focusComponent = btnNo + 1; gNav!.pointee.idleTime = 0; gNav!.pointee.mouseState = .off; playNavigateEffect(); twitchSelection(); return }
    if SwIsNeedDown(kNeed_UINext, ANY_PLAYER) { twitchOutSelection(); btnNo = Int32(PositiveModulo(btnNo + 1, UInt32(MAX_USER_BINDINGS_PER_NEED))); gNav!.pointee.focusComponent = btnNo + 1; gNav!.pointee.idleTime = 0; gNav!.pointee.mouseState = .off; playNavigateEffect(); twitchSelection(); return }
    if SwIsNeedDown(kNeed_UIDelete, ANY_PLAYER) || SwIsKeyDown(Int(CLEAR_BINDING_SCANCODE)) || (gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_MIDDLE))) {
    ...
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); gNav!.pointee.idleTime = 0; gNav!.pointee.menuState = .awaitingPadPress; _ = makePbText(row, btnNo); twitchSelection(); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP_CANCEL); return
    }
}
private func navigateMouseBinding(_ e: UnsafePointer<MenuItem>!) {
    ...
    if SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsKeyDown(Int(SDL_SCANCODE_KP_ENTER.rawValue)) || (gNav!.pointee.mouseState == .hovering && SwIsClickDown(Int(SDL_BUTTON_LEFT))) {
        playStartBindingEffect(); gNav!.pointee.idleTime = 0; gNav!.pointee.menuState = .awaitingMouseClick; _ = makeMbText(row); twitchSelection(); InvalidateAllInputs(); return
    }
}
    ...
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SwIsClickDown(Int(SDL_BUTTON_LEFT)) || SwIsClickDown(Int(SDL_BUTTON_RIGHT)) { doWiggle = true }
        ...
        for sc: Int16 in 0..<Int16(SDL_SCANCODE_COUNT.rawValue) {
            if SwIsKeyDown(Int(sc)) {
                ...
                gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makeKbText(row, keyNo); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP)
                return
            }
        }
        return
    }
    gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makeKbText(row, keyNo); replaceMenuText(STR_CONFIGURE_KEYBOARD_HELP, STR_CONFIGURE_KEYBOARD_HELP); if doWiggle { twitchSelectionNopeWiggle() }
}
private func awaitGamepadPress(_ controller: OpaquePointer) -> Bool {
    ...
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SDL_GetGamepadButton(controller, SDL_GAMEPAD_BUTTON_START) || SwIsClickDown(Int(SDL_BUTTON_LEFT)) || SwIsClickDown(Int(SDL_BUTTON_RIGHT)) { doWiggle = true }
        ...
        for button: Int8 in 0..<Int8(SDL_GAMEPAD_BUTTON_COUNT.rawValue) {
            if SDL_GetGamepadButton(controller, SDL_GamepadButton(rawValue: Int32(button))) {
                ...
                gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP); return true
            }
        }
    ...
    gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_CONFIGURE_GAMEPAD_HELP); if doWiggle { twitchSelectionNopeWiggle() }; return true
}
private func awaitMetaGamepadPress() {
    ...
    if !anyFound { gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makePbText(row, btnNo); replaceMenuText(STR_CONFIGURE_GAMEPAD_HELP, STR_NO_GAMEPAD_DETECTED); twitchSelectionNopeWiggle() }
}
private func awaitMouseClick() {
    ...
    if SwIsKeyDown(Int(SDL_SCANCODE_ESCAPE.rawValue)) || SwIsNeedDown(kNeed_UIConfirm, ANY_PLAYER) || SwIsNeedDown(kNeed_UIBack, ANY_PLAYER) { doWiggle = true }
        ...
        for mb: Int8 in 0..<Int8(NUM_SUPPORTED_MOUSE_BUTTONS) {
            if SwIsClickDown(Int(mb)) { unbindMouseButtonFromAllRemappableInputNeeds(mb); b.pointee.mouseButton = mb; playEndBindingEffect(); gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makeMbText(gNav!.pointee.focusRow); return }
        }
        return
    }
    gNav!.pointee.menuState = .ready; gNav!.pointee.idleTime = 0; _ = makeMbText(gNav!.pointee.focusRow); if doWiggle { twitchSelectionNopeWiggle() }
}
...
private let cMoveMenuDriver: @convention(c) (UnsafeMutablePointer<ObjNode>?) -> Void = { theNode in
    ...
    if gNav!.pointee.menuState == .off { return }
    ...
    if SwIsNeedDown(kNeed_UIStart, ANY_PLAYER) && MenuStyle_GetStartButtonExits(&gNav!.pointee.style) && MenuStyle_GetCanBackOutOfRootMenu(&gNav!.pointee.style)
        && gNav!.pointee.menuState != .awaitingPadPress && gNav!.pointee.menuState != .awaitingKeyPress && gNav!.pointee.menuState != .awaitingMouseClick {
        gNav!.pointee.menuState = .fadeOut
    }
    switch gNav!.pointee.menuState {
    case .fadeIn:
        gNav!.pointee.menuFadeAlpha += gFramesPerSecondFrac * gNav!.pointee.style.fadeInSpeed
        if gNav!.pointee.menuFadeAlpha >= 1.0 { gNav!.pointee.menuFadeAlpha = 1.0; gNav!.pointee.menuState = .ready }
    case .fadeOut:
        if MenuStyle_GetAsyncFadeOut(&gNav!.pointee.style) { gNav!.pointee.menuState = .off }
        else { gNav!.pointee.menuFadeAlpha -= gFramesPerSecondFrac * gNav!.pointee.style.fadeOutSpeed; if gNav!.pointee.menuFadeAlpha <= 0 { gNav!.pointee.menuFadeAlpha = 0; gNav!.pointee.menuState = .off } }
    case .ready:
    ...
    updateArrows()
    if gNav!.pointee.menuState == .off { cleanUpMenuDriver(theNode) }
}
